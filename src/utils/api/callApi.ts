...
export const callApi = async <Response, Data>(options: callApiOptions<Response, Data>) => {
  try {
    const response = await fetch(options.url, options.requestOptions);
    if (!response.ok) {
      const errorBody = await response.text().catch(() => "");
      const errorDetails = errorBody ? ` - ${errorBody}` : "";
      throw new Error(`Request failed with ${response.status} ${response.statusText}${errorDetails}`);
    }
